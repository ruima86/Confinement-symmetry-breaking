function dy = shapeHxi(u,y,para,la,ksig,mu,mub,Ga2,Sig0,sigeqM,Di,ka,sigeq0,Sp,sp,Vext,a,b,Vr)
...
dy(10)=Di.^(-1).*dvu.*h.*sig+ Di.^(-1).*h.^2.*ksig.*sig+ (-1).*Di.^(-1).* ...
  h.^2.*ksig.*(sigeq0+ exp(1).^((-1).*Sp.*((-1/2)+ u).^2).*((-1).* ...
  sigeq0+ sigeqM))+ Di.^(-1).*dsig.*h.*vu+ (-1).*dsig.*h.*r.^(-1).*cos( ...
  p)+ Di.^(-1).*dp.*h.*sig.*v0.*cos(p)+ Di.^(-1).*h.^2.*r.^(-1).*sig.* ...
  vu.*cos(p)+ Di.^(-1).*dsig.*h.*v0.*sin(p)+ Di.^(-1).*h.^2.*r.^(-1).* ...
  sig.*v0.*cos(p).*sin(p);
...
% dy(13)=(1/2).*c0.^2.*ka.*r+ (-1/2).*dp.^2.*h.^(-2).*ka.*r+ (-1).*al.*cos( ...
%   p)+ be.*sin(p)+ (-1).*c0.*ka.*sin(p)+ (1/2).*ka.*r.^(-1).*sin(p).^2; ...